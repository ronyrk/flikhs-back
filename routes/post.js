                    ...
                    Post.findOneAndUpdate({ _id: req.params.id }, { $push: { like: req.user._id } }, { new: true })
                        ...
                        .then(async post => {
                            ...
                            if(post.user._id.toString() != req.user._id.toString()){
                                let noti = await Notification.insertNotification(post.user._id,req.user._id,"like",post.slug)
                                io.in(post.user._id.toString()).emit("newnotification",{notification:noti})
                                //console.log(noti);
                            }
...
route.patch('/delete/:postid', usersignin, (req, res) => {
    ...
    Post.findById(req.params.postid)
        .populate('user', "_id")
        .then(post => {
            if (post.user._id == req.user._id || req.user.role === 'admin') {
